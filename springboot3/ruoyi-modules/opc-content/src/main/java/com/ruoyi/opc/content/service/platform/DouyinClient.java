...
public class DouyinClient implements PlatformClient {
    ...
    private final MockPlatformClient mockFallback = new MockPlatformClient();

    public DouyinClient(DouyinProperties props, ContentTokenEncryptor encryptor) {
        this.props = props;
    ...
    public OAuthToken exchangeCode(String code) {
        try {
            ...
            return mockFallback.exchangeCode(code);
        }
    }
    ...
    public OAuthToken refreshToken(String refreshToken) {
        try {
            ...
            return mockFallback.refreshToken(refreshToken);
        }
    }
    ...
    public String uploadVideo(String accessToken, byte[] videoBytes, String filename) {
        try {
            // 简化: 用 JSON 模拟 (sandbox 一般支持 multipart 也支持 JSON metadata)
            Map<String, Object> meta = new HashMap<>();
            meta.put("access_token", accessToken);
            meta.put("filename", filename);
            meta.put("size", videoBytes == null ? 0 : videoBytes.length);
            String resp = postJson("/video/upload/", mapper.writeValueAsString(meta));
            JsonNode json = mapper.readTree(resp);
            if (json.has("data") && json.get("data").has("video_id")) {
                return json.get("data").get("video_id").asText();
            }
            // sandbox 返回非标准格式, 用 mock
            log.warn("[opc-content] 抖音 uploadVideo 响应无 video_id, 降级 mock");
            return mockFallback.uploadVideo(accessToken, videoBytes, filename);
        } catch (Exception e) {
    ...
    public PublishResult createVideo(String accessToken, String videoId, String title, String[] tags) {
        try {
            ...
            return mockFallback.createVideo(accessToken, videoId, title, tags);
        } catch (Exception e) {
